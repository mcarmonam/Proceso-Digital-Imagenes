...
public class Interfaz extends Application {
    ...
    public void start(Stage primaryStage) throws FileNotFoundException {
        ...
        primaryStage.setTitle("Proceso Digital de Imagenes: Proyecto Final");
	...
	MenuItem filtrodb = new MenuItem("Burkes Dither");
      ...
      Menu mmos = new Menu("Mosaico");
      
      Mosaico mos = new Mosaico();
      
      MenuItem filtromos = new MenuItem("Mosaico");
      filtromos.setOnAction(new EventHandler<ActionEvent>(){
	      public void handle(ActionEvent t) {
		  ImageView imv = (ImageView) grid.getChildren().get(0);
		  Image im = imv.getImage();
		  File dir = selDirectory(primaryStage);
		  
		  ImageView iv;
		  try {
		      iv = new ImageView(mos.mosaico(im,dir,15,15));
		      iv.setFitWidth(500);
		      iv.setPreserveRatio(true);
		      grid.getChildren().remove(1);
		      grid.add(iv,1,0);
      ...
      mmos.getItems().addAll(filtromos);
      
      
      MenuBar mbar = new MenuBar();
      mbar.getMenus().addAll(mfile,mfilt,convm,flm,mdam,efe2,fem,efe3,mfd,mmos);
